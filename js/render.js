...
(function () {
  ...
  var ORDER = ["about", "experience", "speaking", "writing", "research", "contact"];
  ...
  function applyTheme() {
    ...
    var svg = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E" +
      ...
      "%3Ctext x='50' y='66' font-family='Georgia,serif' font-size='" + size + "' font-weight='700' fill='" + col + "' text-anchor='middle'%3E" +
  ...
  function renderProject(P) {
    ...
    if ((P.projects || []).length) {
      ...
      (P.projects || []).forEach(function (item) {
        var card = el("article", "halo__head project-card");
        card.appendChild(el("span", "halo__eyebrow", esc(item.eyebrow || P.eyebrow || "Featured project")));
        var cardName = el("h3", "halo__name");
        cardName.appendChild(el("span", "halo__abbr", esc(item.name)));
        if (item.fullName) cardName.appendChild(el("span", "halo__full", esc(item.fullName)));
        card.appendChild(cardName);
        if (item.tagline) card.appendChild(el("p", "halo__desc", esc(item.tagline)));
        if ((item.highlights || []).length) {
          var itemTags = el("ul", "tags halo__tags");
          item.highlights.forEach(function (h) { itemTags.appendChild(el("li", null, esc(h))); });
          card.appendChild(itemTags);
        }
        if (item.repo) {
          var itemRepo = el("a", "halo__repo");
          itemRepo.href = item.repo; itemRepo.target = "_blank"; itemRepo.rel = "noopener";
          itemRepo.innerHTML = '<svg viewBox="0 0 16 16" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0016 8c0-4.42-3.58-8-8-8z"/></svg><span>View on GitHub</span>';
          card.appendChild(itemRepo);
        }
      ...
      if (P.allUrl) {
        ...
        var all = el("a", "halo__repo");
        all.href = P.allUrl;
        all.innerHTML = '<span>' + esc(P.allLabel || "View all projects") + '</span>';
  ...
  function renderWriting(conf) {
    if (conf.project) {
      ...
      hide("#writing-tabs");
      hide("#panel-writing");
      var foot = document.querySelector(".writing__foot");
  ...
  function renderWordmark() {
    ...
    w.appendChild(el("span", "wordmark__mark", esc(T.faviconLetter || "IP")));
